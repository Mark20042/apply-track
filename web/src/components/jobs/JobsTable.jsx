        ...
        <>
            ...
            <div className="space-y-3 sm:hidden">
                {jobs.map((job) => (
                    <div
                        key={job._id}
                        className="rounded-xl border border-border/60 bg-card shadow-sm p-4 active:bg-muted/30 transition-colors"
                        onClick={() => onEdit(job)}
                    >
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <h3 className="font-semibold text-sm text-foreground truncate">{job.company}</h3>
                                    {job.jobLink && (
                                        <a href={job.jobLink} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-muted-foreground hover:text-primary shrink-0">
                                            <ExternalLink className="h-3.5 w-3.5" />
                                        </a>
                                    )}
                                </div>
                                <p className="text-xs text-muted-foreground truncate mt-0.5">{job.position}</p>
                        ...
                        <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/40">
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
                                    <button className={`inline-flex items-center shadow-sm rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider ${STATUS_BADGE_CLASSES[job.status] || STATUS_BADGE_CLASSES.pending}`}>
                                        {job.status}
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-40">
                                    ...
                                    <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onDelete(job._id); }} className="text-destructive focus:bg-destructive/10 focus:text-destructive cursor-pointer font-medium py-2">
            ...
            {/* Desktop Table View */}
            <div className="rounded-xl border border-border/60 bg-card shadow-sm overflow-hidden hidden sm:block">
